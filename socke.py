...
class Background():
    # das erste image muss das hinterste sein. Denn dieses wird nach oben
...
class Sock():
    ...
    # Geschwindigkeit, die Socke ohne eigene Bewegung hat (zum Beispiel durch
    # bewegende Plattform)
    def basevel(self):
        p = self.onboard()
        if p:
            # XXX: sollte auch y-Geschwindigkeit mitnehmen
            return (p.vel[0], 0)
        else:
            return (0, 0)
    ...
    def update(self, dt):
        ...
        # wichtig, dass die basevel nicht nur am anfang auf ner platform
        # gesetzt wird!!!
        if(self.state == 'still'):
